...
class Post extends Base
{
    ...
    public function comment()
    {
        if (\request()->isPost()) {
            ...
            Db::startTrans();
            $data['floor'] = $m_post->lock(true)->where('first_id', $data['first_id'])->order('id',
                    'desc')->value('floor') + 1;
...
//          仅首层和父层会受到更新的影响
            \addons\bbs\model\Post::whereIn('id',
                [$post->first_id, $id])->update(['post_number'  => Db::raw('post_number + 1'), 'last_time' => time(),
                                                 'last_user_id' => $user_id, 'last_post_id' => $m_post->id
            ]);
